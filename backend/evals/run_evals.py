...
# ===== CONFIGURATION =====
# Defaults target the Docker network service names; override with env vars to run
# from the host against mapped ports (e.g. ANALYZER_URL=http://localhost:8002/analyze).
import os

ANALYZER_URL = os.getenv("ANALYZER_URL", "http://analyseur-offre:8002/analyze")
WRITER_URL = os.getenv("WRITER_URL", "http://redacteur-cv:8003/generate")
...
def generate_cv(
    client: httpx.Client, profile_data: Dict[str, Any], offer_analysis: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    print_info(
        f"📤 Payload keys: user_profile={list(profile_data.keys())[:5]}, offer_analysis={list(offer_analysis.keys())}"
    )

    response = client.post(WRITER_URL, json=payload, timeout=TIMEOUT)
    response.raise_for_status()
    return response.json()
...
def run_evals() -> None:
        ...
        for profile_idx, (profile_name, profile_data) in enumerate(profiles, 1):
            for offer_idx, (offer_name, offer_text) in enumerate(offers, 1):
                ...
                try:
                    ...
                    cv_data = cv_response.get(
                        "cv_data", {}
                    )  # Extract cv_data from response
                    print_success(
                    ...
                    error_msg = (
                        f"{profile_name} × {offer_name}: {type(e).__name__} - {e}"
                    )
                    print_error(error_msg)
                    errors_details.append(error_msg)
                    error_count += 1
    ...
    success_rate = (
        (success_count / total_combinations * 100) if total_combinations > 0 else 0
    )
