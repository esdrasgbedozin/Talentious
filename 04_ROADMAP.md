...
> **État** : backend + frontend faits et testés. Backend : `/billing/checkout-session`, `/billing/webhook` (HMAC vérifiée, idempotent), `/billing/status` ; service isolé ; 9 tests billing (Stripe mocké). **T08 rate limiting** (/auth/login 5/min, 429 RFC 7807, login à temps constant) fait et **vérifié en live** (5×401 → 429). **T09 CORS** : piloté par `CORS_ORIGINS` (env) — vraies URLs au déploiement M6, pas de code. **Frontend T05/T06/T07** : page `/billing` (choix pass → Checkout), succès/annulation, intercepteur 402→/billing (tsc propre). 45 tests backend verts. **Reste seulement** : [PAH-3] validation end-to-end en mode test réel — nécessite les clés Stripe test du fondateur + Stripe CLI (procédure ONBOARDING §6ter). Dette : monté sur `/billing` (pas `/v1`, versioning global à faire) ; webhook ne persiste pas stripe_customer_id ; refunds/litiges non gérés.
...
| M7-T04 | Emails transactionnels — jeu complet | Couvrir TOUS les cas d'usage habituels (contenu FR, sobre, sans tracking) : (1) **bienvenue** après vérification d'adresse ; (2) **confirmation d'achat de pass** (déclenchée par le webhook Stripe `checkout.session.completed`) ; (3) **confirmation de suppression de compte** (RGPD — accusé de traitement) ; (4) **mot de passe modifié** (notification de sécurité, après un reset ou un changement) ; (5) **[optionnel] pass bientôt expiré** (rappel J-3). La vérification (T02) et le reset (T03) ont déjà leurs emails. | `email_service`, `backend/app/routes/billing.py`, `backend/app/routes/account.py`, `backend/app/routes/auth.py` | M7-T01, M7-T02, M7-T03 | 2 h | Chaque événement déclenche l'email correspondant (mock en test) ; achat de pass en test → email de confirmation |
