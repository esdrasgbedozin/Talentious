...
async def generate_cv(request: GenerateRequest):
    ...
    try:
        ...
        logger.info(f"   User: {request.user_profile.personal_info.email}")
        logger.info(f"   Job seniority: {request.offer_analysis.seniority_level}")
        logger.info(f"   Critical skills: {sum(1 for s in request.offer_analysis.hard_skills + request.offer_analysis.soft_skills if s.importance == 'Critical')}")
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Generated CV data validation failed: {str(e)}"
        )
    except Exception as e:
